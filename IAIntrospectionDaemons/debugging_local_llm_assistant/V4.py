...
# 🕷️ Assistant Local LLM V4 - Parsing Robuste des Arguments
...
class LocalLLMLoggerV4:
    """Logger pour l'assistant local LLM V4."""
    
    def __init__(self, session_name: str = None):
        self.session_name = session_name or f"local_llm_v4_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
...
class LocalLLMAssistantV4:
    """
    Assistant V4 avec LLM local et parsing robuste des arguments.
    Version finale : parsing robuste qui gère tous les cas de guillemets.
    """
    
    def __init__(self, name: str = "Local LLM Assistant V4", memory_engine = None):
        self.name = name
        self.logger = LocalLLMLoggerV4(f"{name.lower().replace(' ', '_')}")
    ...
    def _get_system_prompt(self) -> str:
        """Prompt système pour l'assistant V4."""
...
Sinon, réponds normalement avec tes observations."""
    ...
    def _clean_argument_value(self, value: str) -> str:
        ...
        # Supprimer les guillemets simples au début et à la fin
        while value.startswith("'") and value.endswith("'"):
            value = value[1:-1].strip()
        
        return value
    
    def _parse_arguments_robust(self, args_text: str) -> Dict[str, str]:
    ...
    def _extract_action(self, response: str) -> Optional[Dict[str, Any]]:
        """Extrait une action de la réponse du LLM avec parsing robuste."""
        ...
        if match:
            ...
            # Parser les arguments avec la méthode robuste
            arguments = self._parse_arguments_robust(args_text)
...
def create_local_llm_assistant_v4(name: str = "Local LLM Assistant V4", memory_engine = None) -> LocalLLMAssistantV4:
    """Factory pour créer un assistant local LLM V4."""
    return LocalLLMAssistantV4(name, memory_engine)


def main():
    """Test de l'assistant local LLM V4."""
    print("🕷️ Test de l'Assistant Local LLM V4")
    print("=" * 50)
    
    # Créer l'assistant
    from MemoryEngine.core.engine import MemoryEngine
    memory_engine = MemoryEngine()
    assistant = create_local_llm_assistant_v4("Alma Local LLM V4", memory_engine)
