...
# 🕷️ Assistant Local LLM V3 - Parsing Amélioré des Arguments
...
class LocalLLMLoggerV3:
    """Logger pour l'assistant local LLM V3."""
    
    def __init__(self, session_name: str = None):
        self.session_name = session_name or f"local_llm_v3_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
...
class LocalLLMAssistantV3:
    """
    Assistant V3 avec LLM local et parsing amélioré des arguments.
    Version améliorée : meilleur parsing des arguments et gestion des guillemets.
    """
    
    def __init__(self, name: str = "Local LLM Assistant V3", memory_engine = None):
        self.name = name
        self.logger = LocalLLMLoggerV3(f"{name.lower().replace(' ', '_')}")
    ...
    def _get_system_prompt(self) -> str:
        """Prompt système pour l'assistant V3."""
...
Sinon, réponds normalement avec tes observations."""
    ...
    def _clean_argument_value(self, value: str) -> str:
        """Nettoie une valeur d'argument en supprimant les guillemets superflus."""
    ...
    def _parse_arguments(self, args_text: str) -> Dict[str, str]:
        """Parse les arguments avec gestion améliorée des guillemets."""
        arguments = {}
        ...
        try:
            # Utiliser shlex pour parser correctement les arguments
            parsed_args = shlex.split(args_text)
            
            for arg in parsed_args:
                if '=' in arg:
                    key, value = arg.split('=', 1)
                    arguments[key] = self._clean_argument_value(value)
                else:
                    # Argument sans valeur (booléen)
                    arguments[arg] = True
                    
        except Exception as e:
            # Fallback : parsing simple avec regex
            print(f"⚠️ Parsing shlex échoué, fallback regex: {e}")
    ...
    def _extract_action(self, response: str) -> Optional[Dict[str, Any]]:
        """Extrait une action de la réponse du LLM avec parsing amélioré."""
        ...
        if match:
            ...
            # Parser les arguments avec la nouvelle méthode
...
def create_local_llm_assistant_v3(name: str = "Local LLM Assistant V3", memory_engine = None) -> LocalLLMAssistantV3:
    """Factory pour créer un assistant local LLM V3."""
    return LocalLLMAssistantV3(name, memory_engine)


def main():
    """Test de l'assistant local LLM V3."""
    print("🕷️ Test de l'Assistant Local LLM V3")
    print("=" * 50)
    
    # Créer l'assistant
    from MemoryEngine.core.engine import MemoryEngine
    memory_engine = MemoryEngine()
    assistant = create_local_llm_assistant_v3("Alma Local LLM V3", memory_engine)
