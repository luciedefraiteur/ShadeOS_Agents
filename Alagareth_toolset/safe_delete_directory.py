...
def safe_delete_directory(path: str, recursive: bool = False, debug: bool = False) -> bool:
    ...
    if debug:
        print(f"[DEBUG - safe_delete_directory] Tentative de suppression de répertoire: {path} (récursif: {recursive})", file=sys.stderr)
    ...
    try:
        ...
        if debug:
            print(f"[DEBUG - safe_delete_directory] {error_msg}", file=sys.stderr)
        print(error_msg, file=sys.stderr)
        return False
    except OSError as e:
        error_msg = f"Erreur lors de la suppression du répertoire '{path}': {e}"
        if debug:
            print(f"[DEBUG - safe_delete_directory] {error_msg}", file=sys.stderr)
        print(error_msg, file=sys.stderr)
        return False
...
if __name__ == "__main__":
    ...
    result = safe_delete_directory(args.path, args.recursive, debug=args.debug)
    print(result)
